...
% 2020.3.6 compress with start position
function mtspf_ga(varargin)
    xy          =   100*rand(61,2); % 60 sensor nodes position and start/end position
    xy(1,:)     =   [0 0];         %start/end position(0,0)
    dmat        =   [];            %distance matrix for all sensors
    nSalesmen   =   4;             %mutiple salesmen 
    minTour     =   2;             %min tour constraint
    popSize     =   80;            %particle size or population
    numIter     =   5e3;           %the number of iterate
    showProg    =   true;          %show the figure

    if isempty(dmat)               %distance matrix calculation
        nPoints = size(xy,1);      %distance matrix size
        a = meshgrid(1:nPoints);   %meshgrid
        dmat = reshape(sqrt(sum((xy(a,:)-xy(a',:)).^2,2)),nPoints,nPoints);
    end

    N=nPoints;  %senors and start/end position sum
    n   =N-1;   %sensor number
    ...
    if showProg
        figure('Name','MTSP for WRSN| Current Best Solution','Numbertitle','off');
        hAx = gca;
    end     
    for iter = 1:numIter       
    ...
    function breaks = rand_breaks() % Force Breaks to be at Least the Minimum Tour Length
